...
try:
    import trl
except ModuleNotFoundError as exc:  # pragma: no cover - helpful error when editable install missing
    raise RuntimeError(
        "The `trl` package must be installed (pip install -e /path/to/trl) before importing open_r1 configs."
    ) from exc
...
@dataclass
class ADPOConfig(trl.ADPOConfig):
    """ADPO configuration extended with Open-R1 specific logging knobs."""
...
@dataclass
class ADPOScriptArguments(GRPOScriptArguments):
